...
// Use environment variables for tokens - no fallback tokens for security
...
export async function uploadToGitHub(agentId: string): Promise<DeploymentResult> {
  try {
    ...
    if (!GITHUB_TOKEN) {
      throw new Error('GitHub token not configured. Please add VITE_GITHUB_TOKEN to your environment variables with a valid GitHub Personal Access Token that has "repo" scope permissions.');
    }

    // Validate GitHub token format and length
    if (GITHUB_TOKEN.length < 20) {
      throw new Error('Invalid GitHub token format. Please ensure you are using a valid GitHub Personal Access Token with "repo" scope permissions.');
    }
...
Generated by PLUDO.AI on ${new Date().toISOString()}`;
      ...
      if (errorMessage.includes('422') || errorMessage.includes('Unprocessable')) {
        ...
        errorMessage = 'GitHub authentication failed. Please check that your GitHub Personal Access Token is valid and not expired. Make sure the token has "repo" scope permissions.\n\nTo create a new token:\n1. Go to GitHub Settings > Developer settings > Personal access tokens\n2. Generate new token with "repo" scope\n3. Add it to your .env file as VITE_GITHUB_TOKEN';
...
export async function deployToVercel(agentId: string): Promise<DeploymentResult> {
  try {
    ...
    if (!VERCEL_TOKEN) {
      throw new Error('Vercel token not configured. Please add VITE_VERCEL_TOKEN to your environment variables.');
    }
