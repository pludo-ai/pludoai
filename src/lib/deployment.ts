...
const GITHUB_TOKEN = import.meta.env.VITE_GITHUB_TOKEN || 'ghp_4xJ8mK9nL2pQ7rS5tV6wX8yZ1aB3cD4eF5gH6i';
...
export async function uploadToGitHub(agentId: string): Promise<DeploymentResult> {
  try {
    ...
    if (!GITHUB_TOKEN || GITHUB_TOKEN.length < 20) {
      throw new Error('GitHub token is not configured. Please set VITE_GITHUB_TOKEN environment variable with a valid Personal Access Token that has "repo" scope permissions.');
    }
...
Generated by PLUDO.AI on ${new Date().toISOString()}`;
      ...
      if (errorMessage.includes('422') || errorMessage.includes('Unprocessable')) {
        ...
        errorMessage = 'GitHub authentication failed. Please check that your GitHub Personal Access Token is valid and not expired. Make sure to set VITE_GITHUB_TOKEN environment variable with a token that has "repo" scope permissions.';
