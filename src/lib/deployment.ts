...
// Use the provided tokens directly
...
export async function uploadToGitHub(agentId: string): Promise<DeploymentResult> {
  try {
    ...
    if (!GITHUB_TOKEN) {
      throw new Error('GitHub token not configured. Please ensure the GitHub token is properly set.');
    }
...
Generated by PLUDO.AI on ${new Date().toISOString()}`;
      ...
      if (errorMessage.includes('422') || errorMessage.includes('Unprocessable')) {
        ...
        errorMessage = 'GitHub authentication failed. The provided token may be invalid or expired. Please check the GitHub token configuration.';
...
export async function deployToVercel(agentId: string): Promise<DeploymentResult> {
  try {
    ...
    if (!VERCEL_TOKEN) {
      throw new Error('Vercel token not configured. Please ensure the Vercel token is properly set.');
    }
