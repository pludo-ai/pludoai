...
// Helper function to generate likely Vercel URL
function generateVercelUrl(repoName: string): string {
...
export async function triggerDeploymentWithFile(agentId: string): Promise<DeploymentResult> {
  try {
    ...
    // Generate the likely Vercel URL
    const vercelUrl = generateVercelUrl(repoName);
    
    // Update database with the generated URL
    const { error: updateError } = await supabase
      .from('agents')
      .update({
    ...
    if (updateError) {
      console.error('Warning: Failed to update agent with Vercel URL:', updateError);
    }
...
`;
    ...
    return {
      ...
      vercelUrl: vercelUrl,
      embedCode: `<!-- ${agent.brand_name} AI Assistant - Generated by PLUDO.AI -->
...
export async function deployToVercel(agentId: string): Promise<DeploymentResult> {
  try {
    ...
    try {
      ...
      // IMMEDIATELY trigger deployment after project creation
      console.log('🚀 Immediately triggering deployment with trigger.txt...');
      
      // Generate the likely Vercel URL
      const vercelUrl = generateVercelUrl(repoName);
      
      // Update database with the generated URL first
      const { error: updateError } = await supabase
        .from('agents')
        .update({
          vercel_url: vercelUrl,
      ...
      // Create GitHub service instance and push trigger file
      const github = new GitHubService(GITHUB_TOKEN);
      
      // Create trigger file content
      const triggerContent = `# Deployment Trigger

This file was created to trigger a new deployment immediately after Vercel project creation.
...
`;
      ...
      // Upload trigger file immediately
      await github.uploadFiles(
        repoFullName,
        [{
          path: 'trigger.txt',
        ...
        `🚀 Auto-trigger deployment after Vercel project creation - ${new Date().toISOString()}`
      );

      console.log('✅ Auto-trigger file pushed successfully');
...
<script src="${vercelUrl}/float.js" defer></script>`;
      ...
      console.log('Vercel project creation issue:', vercelError.message);
      
      // If project already exists or other Vercel issues, still try to trigger
      const vercelUrl = generateVercelUrl(repoName);
      
      // Update database with generated URL
      const { error: updateError } = await supabase
        .from('agents')
        .update({
          vercel_url: vercelUrl,
      ...
      // Still try to push trigger file even if Vercel project creation had issues
      try {
        console.log('🚀 Pushing trigger file despite Vercel issues...');
        
        const github = new GitHubService(GITHUB_TOKEN);
        
        const triggerContent = `# Deployment Trigger

This file was created to trigger deployment after Vercel setup.
...
`;
        ...
        await github.uploadFiles(
          ...
          `🚀 Trigger deployment - ${new Date().toISOString()}`
        );

        console.log('✅ Trigger file pushed successfully despite Vercel issues');
      } catch (triggerError) {
        console.error('Failed to push trigger file:', triggerError);
      }
      ...
      return {
        ...
        githubRepo: agent.github_repo,
        vercelUrl: vercelUrl,
        error: 'Project setup completed. Deployment triggered automatically.',
      };
    }
