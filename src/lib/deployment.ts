...
export async function deployToVercel(agentId: string): Promise<DeploymentResult> {
  try {
    ...
    try {
      ...
      try {
        const vercelUrl = await vercel.waitForDeployment(project.id, 60000); // 1 minute
...
<script src="${vercelUrl}/float.js" defer></script>`;
        ...
        console.log('Deployment needs manual trigger:', deploymentError.message);
        ...
        return {
          ...
          githubRepo: agent.github_repo,
          vercelUrl: vercelUrl,
          error: 'Deployment is being prepared. You can trigger it manually.',
        };
      }
    } catch (vercelError: any) {
      console.log('Vercel project creation issue:', vercelError.message);
      
      // If project already exists or other Vercel issues, generate URL
      const vercelUrl = generateVercelUrl(repoName);
      ...
      return {
        ...
        githubRepo: agent.github_repo,
        vercelUrl: vercelUrl,
        error: 'Project setup completed. Manual trigger may be needed.',
      };
    }
