...
export const Create: React.FC = () => {
  ...
  // Store original form data for edit mode comparison
  ...
  // Trigger deployment timer state
  ...
  const loadAgentForEditing = async (agentId: string) => {
    try {
      ...
      if (agent) {
        ...
        // Set deployment result for existing agent
        setDeploymentResult({
          agentId: agentId,
...
<script src="${agent.vercel_url}/widget.js" defer></script>` : undefined
        ...
        toast.success('Agent loaded for editing');
      }
    } catch (error: any) {
  ...
  const validateSubdomainAvailability = async (subdomain: string) => {
    ...
    setSubdomainValidation(prev => ({ ...prev, isValidating: true }));
  ...
  const handleDeployToCloud = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Start the 45-second timer for trigger deployment button
        startTriggerTimer();
      } else {
        updateDeploymentStep('step-3', 'error', result.error);
        toast.error('Failed to deploy to cloud');
        
        // Show trigger button immediately if deployment failed but project exists
        if (result.vercelUrl) {
          startTriggerTimer();
        }
      }
  ...
  const handleUpdateAgent = async () => {
    ...
    try {
      ...
      updateDeploymentStep('step-2', 'loading', 'Pushing changes to repository...');

      // Regenerate and push to repository
      const result = await regenerateAgent(currentAgentId);

      if (result.success) {
        updateDeploymentStep('step-2', 'success', 'Changes pushed to repository');
        updateDeploymentStep('step-3', 'success', 'Auto-deployment triggered');
                ...
                <div className="mt-6">
                    ...
                    <div className="flex-1">
                      <div className="flex">
                        <Input
                          ...
                          required
                        />
                        <div className="px-3 py-2 bg-gray-100 dark:bg-gray-800 border border-l-0 border-gray-300 dark:border-gray-600 rounded-r-lg text-gray-600 dark:text-gray-400 text-sm flex items-center">
                          .pludo.online
                        </div>
                      </div>
                    </div>
                    {!isEditMode && formData.subdomain && (
                      <Button
                        variant="ghost"
                        size="sm"
                  ...
                  {!isEditMode && (
                    <div className="mt-2">
                      ...
                      {!subdomainValidation.isValidating && formData.subdomain && (
                        <>
                          {subdomainValidation.isValid && subdomainValidation.isAvailable && (
                            <p className="text-sm text-green-600 dark:text-green-400 flex items-center">
                              <CheckCircle className="w-4 h-4 mr-1" />
                              {formData.subdomain}.pludo.online is available!
                            </p>
                          )}
                            ...
                            <div className="text-sm text-red-600 dark:text-red-400">
                              ...
                              {subdomainValidation.suggestion && (
                                <p className="mt-1">
                                  ...
                                  >
                                    {subdomainValidation.suggestion}.pludo.online
                                  </button>
                                </p>
                              )}
                            </div>
                  ...
                  <div className="space-y-3">
                    {isEditMode ? (
                      <Button
                        ...
                        disabled={isGenerateCodeDisabled || !hasFormChanged()}
                      ...
                      <>
                        ...
                        {/* Trigger Deployment Button with 45-second delay */}
